...
use nalgebra::Dyn;
use sssp_fast::{
    AdjListGraph, Graph, PARENT_NONE, RelaxResult, SsspBuffers, cheeky_dijkstra, relax_cond,
};
...
fn test_dijkstra_ignore_meta() {
    ...
    cheeky_dijkstra(&meta_g, 0, &mut meta_buf);
    cheeky_dijkstra(&plain_g, 0, &mut plain_buf);
...
fn test_relax_cond() {
    ...
    let mut parent = vec![PARENT_NONE, PARENT_NONE];
...
fn test_meta_gated_routing() {
    ...
    let mut parent = vec![PARENT_NONE; 3];
    dist[0] = 0.0;
